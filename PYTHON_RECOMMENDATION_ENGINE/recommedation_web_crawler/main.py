...
def scrape_naukri_jobs(num_pages=1):
    ...
    with open('recommedation_web_crawler/job_details.csv', 'w', newline='', encoding='utf-8') as csvfile:
...
def web_crawler_main():
    get_in_demand_skills(num_pages=10)
    df = pd.read_csv("recommedation_web_crawler/Online_Courses.csv")
    df = df[["Title", "URL", "Skills"]]
    csv_file = 'recommedation_web_crawler/job_details.csv' 
    ...
    with open('recommedation_web_crawler/today_job_trend_course.csv','w',newline='', encoding='utf-8') as job_trends:
